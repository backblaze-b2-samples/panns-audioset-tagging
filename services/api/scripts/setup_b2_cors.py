...
existing CORS rules are preserved and one rule (ID `panns-audioset-tagging-direct-upload`) is
...
RULE_ID = "panns-audioset-tagging-direct-upload"
...
def _client():
    ...
    # client leaves region unset for object ops. B2_REGION is the single source
    # of truth for both the region and the derived endpoint.
    return boto3.client(
        "s3",
        ...
        config=Config(signature_version="s3v4", user_agent_extra="b2ai-panns-audioset-tagging"),
    )
